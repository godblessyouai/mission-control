...
urgency_f = st.sidebar.multiselect("Urgency", ["Low", "Medium", "High", "Critical"], default=["Low", "Medium", "High", "Critical"])
status_f = st.sidebar.multiselect("Task status", ["Open", "In Progress", "Blocked", "Done", "Snoozed"], default=["Open", "In Progress", "Blocked", "Done", "Snoozed"])
    ...
    with st.expander("⚡ Quick Actions"):
        ...
        with a1:
            if st.button("Mark Done"):
                update_task(int(task_id), {"status": "Done"})
                st.success("Updated")
                st.rerun()
        with a2:
            if st.button("Snooze +1 day"):
                update_task(int(task_id), {"status": "Snoozed", "snooze_until": (datetime.now() + timedelta(days=1)).date().isoformat()})
                st.success("Updated")
                st.rerun()
        with a3:
            new_owner = st.text_input("Reassign owner")
            if st.button("Reassign") and new_owner.strip():
                update_task(int(task_id), {"owner": new_owner.strip()})
                st.success("Updated")
                st.rerun()
        with a4:
            new_due = st.date_input("New deadline", key="newdue")
            if st.button("Set deadline"):
                update_task(int(task_id), {"due_date": new_due.isoformat()})
                st.success("Updated")
                st.rerun()
...
with tab_escalations:
    ...
    esc = fetch_df(
        ...
        ORDER BY urgency DESC, COALESCE(due_date,'9999-12-31')
        """
    )
